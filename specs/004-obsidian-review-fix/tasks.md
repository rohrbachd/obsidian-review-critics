...
- [ ] T001 Reconcile release-remediation scope references in `docs/review-critic-obsidian-review-fix-plan.md` and `specs/004-obsidian-review-fix/quickstart.md`
- [ ] T002 [P] Create metadata/readiness regression test scaffolds in `tests/unit/release-metadata.unit.test.ts` and `tests/unit/release-workflow.unit.test.ts`
- [ ] T003 [P] Create release-readiness integration test scaffold in `tests/integration/us-release-readiness.integration.test.ts`
...
- [ ] T004 Add 2026-05-07 canonical review-finding baseline entries (`RF-001` through `RF-007`) in `docs/review-critic-obsidian-review-fix-plan.md`
- [ ] T005 [P] Extend compliance guard coverage for flagged API/DOM patterns in `tests/unit/obsidian-bot-compliance.unit.test.ts`
- [ ] T006 [P] Add release metadata consistency assertions to `scripts/check-obsidian-release.mjs` and `tests/unit/release-metadata.unit.test.ts`
...
- [ ] T007 [P] [US1] Implement metadata consistency tests for version and min-app mapping in `tests/unit/release-metadata.unit.test.ts`
- [ ] T008 [P] [US1] Add release-check validation case for `1.1.1` in `tests/integration/us-release-readiness.integration.test.ts`

### Implementation for User Story 1

- [ ] T009 [US1] Update plugin release metadata to `1.1.1` and `1.7.2` in `manifest.json`
- [ ] T010 [US1] Update package release version to `1.1.1` in `package.json`
- [ ] T011 [US1] Append compatibility mapping `"1.1.1": "1.7.2"` in `versions.json`
- [ ] T012 [US1] Ensure release-check logic validates the new metadata/tag pair in `scripts/check-obsidian-release.mjs`
- [ ] T013 [US1] Document blocker-resolution evidence for `RF-001` compatibility mismatch in `docs/review-critic-obsidian-review-fix-plan.md`
...
- [ ] T014 [P] [US2] Add helper-usage rendering regression assertions in `tests/unit/live-preview.unit.test.ts` and `tests/unit/reading-view.unit.test.ts`
- [ ] T015 [P] [US2] Add comments-pane structure assertions for helper migration in `tests/unit/comments-view.unit.test.ts`
- [ ] T016 [P] [US2] Add popout-safe document-owner behavior coverage in `tests/integration/us3-rendering.integration.test.ts`

### Implementation for User Story 2

- [ ] T017 [US2] Replace deprecated module-list import usage with Node native builtins in `esbuild.config.mjs`
- [ ] T018 [US2] Remove `builtin-modules` dependency and refresh lockfile in `package.json` and `package-lock.json`
- [ ] T019 [US2] Add direct `@codemirror/state` and `@codemirror/view` dependencies in `package.json` and `package-lock.json`
- [ ] T020 [US2] Replace `createEl('div', ...)` with `createDiv(...)` in flagged comment-pane blocks in `src/comments-view.ts`
- [ ] T021 [US2] Replace direct element creation with helper APIs in `src/live-preview.ts`
- [ ] T022 [US2] Replace document-dependent rendering calls with helper/owner-document APIs in `src/reading-view.ts`
- [ ] T023 [US2] Replace global document-root access with active-document-compatible root handling in `src/main.ts`
- [ ] T024 [US2] Update warning-remediation evidence status for `RF-002` through `RF-007` in `docs/review-critic-obsidian-review-fix-plan.md`
...
- [ ] T025 [P] [US3] Add workflow-gate assertions for release checks and artifact list in `tests/unit/release-workflow.unit.test.ts`
- [ ] T026 [P] [US3] Add release-readiness integration assertion for required provenance artifacts in `tests/integration/us-release-readiness.integration.test.ts`

### Implementation for User Story 3

- [ ] T027 [US3] Create tag-triggered attested release workflow in `.github/workflows/release.yml`
- [ ] T028 [US3] Align release-check behavior and workflow gate expectations in `scripts/check-obsidian-release.mjs` and `.github/workflows/release.yml`
- [ ] T029 [US3] Document automated release + attestation verification process in `docs/release-workflow.md` and `docs/publication-checklist.md`
- [ ] T030 [US3] Record resubmission evidence procedure and contract linkage in `specs/004-obsidian-review-fix/quickstart.md` and `specs/004-obsidian-review-fix/contracts/release-readiness.openapi.yaml`
...
- [ ] T031 [P] Run full validation sequence for `1.1.1` and capture outcomes in `specs/004-obsidian-review-fix/quickstart.md`
- [ ] T032 Prepare final publication/resubmission notes in `docs/Obsidian Publication Guide.md`
- [ ] T033 Reconcile final spec/plan/tasks consistency after performance delta capture in `specs/004-obsidian-review-fix/spec.md`, `specs/004-obsidian-review-fix/plan.md`, and `specs/004-obsidian-review-fix/tasks.md`
- [ ] T034 Run targeted performance regression checks in `tests/perf/us1-track-typing-latency.perf.test.ts` and `tests/perf/us3-pane-refresh.perf.test.ts`, and record baseline deltas in `specs/004-obsidian-review-fix/quickstart.md`
